...
- `Stop` clears scheduled playback and resets the current playhead to the first active loop/reset start;
- `Reset` returns the current playhead to the first active loop/reset start;
...
- first-active loop start/current playhead behavior;
...
- media segment and contribution tile click-to-cue behavior;
- pause preserving the playhead and stop resetting to the first active contribution step;
